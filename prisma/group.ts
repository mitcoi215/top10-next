...
async function main() {
  console.log('🌱 Start authoritative category-group seeding...');
  ...
  for (const groupData of categoryGroupsData) {
    ...
    console.log(`✓ Upserted group: ${group.slug}`);

    const allowedSlugs = categories.map((c) => c.slug);

    // 2️⃣ REMOVE category KHÔNG nằm trong danh sách
    const removed = await prisma.category.updateMany({
      where: {
        groupId: group.id,
    ...
    if (removed.count > 0) {
      console.log(`  🗑 Removed ${removed.count} categories from group`);
    }

    // 3️⃣ Upsert đúng category + set order
    let order = 1;
    for (const cat of categories) {
      ...
      console.log(`  ✓ Synced category: ${cat.slug}`);
      order++;
    }
  }

  console.log('✅ Authoritative seed completed!');
}
