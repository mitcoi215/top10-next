...
// Function to check if a CSS rule is valid
function isValidCssRule(css) {
  // Check for invalid pseudo-element selectors
  if (css.match(/:after\s+[a-z]/i)) return false;
  if (css.match(/:before\s+[a-z]/i)) return false;
  if (css.match(/::after\s+[a-z]/i)) return false;
  if (css.match(/::before\s+[a-z]/i)) return false;

  // Check for JavaScript code
  if (css.includes('function(')) return false;
  if (css.includes('return ')) return false;
...
let outputCss = '/* CSS for Category Page Components - Extracted from template v3 */\n\n';
